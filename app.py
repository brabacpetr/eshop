...
def produkty():
    ...
    return render_template(
        ...
        kategorie=kategorie,
        aktivni_kategorie_id=aktivni_kategorie_id,
    )
...
def pridat_do_kosiku():
    ...
    databaze = ziskat_db()
    produkt = databaze.execute(
        "SELECT * FROM produkt WHERE id = ?", (produkt_id,)
    ).fetchone()
    databaze.close()
...
def pouzit_kupon():
    ...
    if (
        kupon["limit_pouziti"] is not None
        and kupon["pocet_pouziti"] >= kupon["limit_pouziti"]
    ):
    ...
    flash(
        f"Kupón {kupon['kod']} byl uplatněn — sleva {kupon['sleva_procent']} %.",
        "success",
    )
    return redirect(url_for("zobraz_kosik"))
...
STATICKE_STRANKY = {
    "podminky": {
        ...
        "sekce": [
            (
                ...
                "při nákupu zboží prostřednictvím internetového obchodu ChytniTo.",
            ),
            (
    ...
    "soukromi": {
        ...
        "sekce": [
            ...
            (
                "Správce údajů",
                "Správcem osobních údajů je provozovatel e-shopu ChytniTo. Zpracování "
...
def admin_prehled():
    databaze = ziskat_db()
    pocet_produktu = databaze.execute("SELECT COUNT(*) as n FROM produkt").fetchone()[
        "n"
    ]
    pocet_objednavek = databaze.execute(
        "SELECT COUNT(*) as n FROM objednavka"
    ).fetchone()["n"]
...
def admin_novy_produkt():
    ...
    if request.method == "GET":
        databaze.close()
        return render_template(
            "admin/formular_produktu.html", kategorie=kategorie, produkt=None, chyby={}
        )
...
def admin_detail_objednavky(objednavka_id):
    ...
    platne_stavy = ("nova", "zaplacena", "odeslana", "dokoncena", "zrusena")
    return render_template(
        "admin/detail_objednavky.html",
        objednavka=objednavka,
        polozky=polozky,
        kupon=kupon,
        cena_dopravy=cena_dopravy,
        poplatek_dobirka=poplatek_dobirka,
        platne_stavy=platne_stavy,
    )
...
def admin_kupony():
    ...
    kupony = databaze.execute("SELECT * FROM slevovy_kupon ORDER BY id DESC").fetchall()
    databaze.close()
    return render_template(
        "admin/kupony.html", kupony=kupony, chyby=chyby, formular=request.form
    )
